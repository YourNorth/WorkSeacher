pipeline {
    agent {
        dockerfile {
            ...
            args '-v /home/.m2:/root/.m2 -v /home/maven-image/id_rsa:/root/.ssh/id_rsa -v /var/run/docker.sock:/var/run/docker.sock --network host'
        }
    }
    stages {
        stage('Build') {
        ...
        stage('Deliver') { 
            steps {
                dir( 'work_sercher'){
                    ...
                    sh 'sshpass -p "SidikovCrasavaqwert007" ssh developer@172.18.0.1 -o StrictHostKeyChecking=no -p 12 "cd /home/developer/GitHub/WorkSeacher/work_sercher; mvn spring-boot:build-image -DskipTests -Prelease -Dspring-boot.build-image.imageName=springio/gs-spring-boot-docker;"'   
                }
            }
        }	
        stage('Start server'){
